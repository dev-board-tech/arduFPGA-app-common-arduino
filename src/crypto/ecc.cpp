...
int ecc_make_key(uint8_t p_publicKey[ECC_BYTES * 2], uint8_t p_privateKey[ECC_BYTES])
{
	...
	ecc_native2bytes(p_publicKey + 32, l_public.y);
    return 1;
}
...
#if 0
int ecdsa_sign(const uint8_t p_privateKey[ECC_BYTES], const uint8_t p_hash[NUM_ECC_DIGITS], uint8_t p_signature[ECC_BYTES*2])
{
	...
	ecc_native2bytes(p_signature, r);
    ecc_native2bytes(p_signature + ECC_BYTES, s);
