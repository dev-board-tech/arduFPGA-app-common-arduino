...
#define VS_SCI_AICTRL1		0x0D	//RW	Application control register 1
#define VS_SCI_AICTRL2		0x0E	//RW	Application control register 2
#define VS_SCI_AICTRL3		0x0F	//RW	Application control register 3

#define VS_SM_DIFF           (1<< 0)
#define VS_SM_LAYER12        (1<< 1) /* VS1063, VS1053, VS1033, VS1011 */
#define VS_SM_RECORD_PATH    (1<< 1) /* VS1103 */
#define VS_SM_RESET          (1<< 2)
#define VS_SM_CANCEL         (1<< 3) /* VS1063, VS1053 */
#define VS_SM_OUTOFWAV       (1<< 3) /* VS1033, VS1003, VS1011 */
#define VS_SM_OUTOFMIDI      (1<< 3) /* VS1103 */
#define VS_SM_EARSPEAKER_LO  (1<< 4) /* VS1053, VS1033 */
#define VS_SM_PDOWN          (1<< 4) /* VS1003, VS1103 */
#define VS_SM_TESTS          (1<< 5)
#define VS_SM_STREAM         (1<< 6) /* VS1053, VS1033, VS1003, VS1011 */
#define VS_SM_ICONF          (1<< 6) /* VS1103 */
#define VS_SM_EARSPEAKER_HI  (1<< 7) /* VS1053, VS1033 */
#define VS_SM_DACT           (1<< 8)
#define VS_SM_SDIORD         (1<< 9)
#define VS_SM_SDISHARE       (1<<10)
#define VS_SM_SDINEW         (1<<11)
#define VS_SM_ENCODE         (1<<12) /* VS1063 */
#define VS_SM_ADPCM          (1<<12) /* VS1053, VS1033, VS1003 */
#define VS_SM_EARSPEAKER1103 (1<<12) /* VS1103 */
#define VS_SM_ADPCM_HP       (1<<13) /* VS1033, VS1003 */
#define VS_SM_LINE1          (1<<14) /* VS1063, VS1053 */
#define VS_SM_LINE_IN        (1<<14) /* VS1033, VS1003, VS1103 */
#define VS_SM_CLK_RANGE      (1<<15) /* VS1063, VS1053, VS1033 */
#define VS_SM_ADPCM_1103     (1<<15) /* VS1103 */
...
class vs10xx {
	...
	uint16_t regRead(unsigned char reg);
	void softReset();
	void hardReset();
	void setPll(uint32_t qFreq);
	void setVolume(uint8_t r, uint8_t l);
	uint16_t getVolume();
	void setBas(uint8_t bl, uint8_t bc, uint8_t tl, uint8_t tc);
	void sendCancel();
	void sendNull(uint16_t len);
	bool send32BData(uint8_t *buffer);
	bool send1byteData(uint8_t data);
private:
	inline void assertRst();
	inline void deassertRst();
	inline void assertDcs();
	inline void deassertDcs();
	inline void assertCs();
	inline void deassertCs();
	inline void checkBusy();
	inline bool checkBusySkip();
